...
func Update() {
	...
	utils.RunCmd(exec.Command("go", "clean", "-i", "github.com/topofstack/cutego/cmd/..."), "run \"go clean cmd\"")
	utils.RunCmd(exec.Command("go", "clean", "-i", "github.com/topofstack/cutego/internal/..."), "run \"go clean internal\"")
	...
	utils.RunCmd(exec.Command("go", "install", "-v", fmt.Sprintf("-ldflags=\"-X=github.com/topofstack/cutego/internal/cmd.buildVersion=%v\"", hash), fmt.Sprintf("github.com/topofstack/cutego/cmd/...")), "run \"go install\"")
...
func Upgrade() {
	...
	utils.RunCmd(exec.Command("go", "clean", "-i", "github.com/topofstack/cutego/..."), "run \"go clean\"")
	utils.RemoveAll(utils.GoQtPkgPath())

	utils.RunCmd(exec.Command("go", "get", "-v", "-d", "github.com/topofstack/cutego/cmd/..."), "run \"go get\"")
